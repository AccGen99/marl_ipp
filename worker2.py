...
class Worker:
    ...
    def run_episode(self, currEpisode):
        ...
        for i in range(256):
            ...
            for active_agent in active_agent_ids: # Execute only for agents who have budget > 0.0 and haven't collided
                ...
                # neib_encoding = torch.FloatTensor(neib_encoding).unsqueeze(0).to(self.device)
                ...
                # agent.experience[13] += neib_encoding
            ...
            # for res in results:
                ...
                reward, done = env_copy_copy.step_sample(agent, action, save_img=False) # Get marginalized rewards for each agent
                agent.ipp_reward = reward
                # agent.experience[5] += torch.FloatTensor([[[reward]]]).to(self.device)
                _ = self.env.step_sample(agent, action, save_img=False) # Update the environment
            # p.close()

            self.sample_size = len(agent.action_coords)
            min_dist = self.env.post_processing(self.save_image)
            ...
            for agent_id in active_agent_ids:
                ...
                # agent.experience[5] += torch.FloatTensor([[[agent.reward]]]).to(self.device)
            ...
            if self.env.budget < 0.0 or len(active_agent_ids) == 0:
                ...
                for agent in agents_list:
                    ...
                    if agent.collided:
                        agent.experience[6].append(torch.FloatTensor([[-5.0]]).to(self.device)) # Negative reward for collision based episode termination
                    else:
                ...
                perf_metrics['detection_rate'] = 100*self.env.detected_fruits
                perf_metrics['collision'] = self.env.collision

                print('{} Goodbye world! We did it!'.format(i))
                break

        print('Episode {} completed - {:.2f}%'.format(self.currEpisode, 100*self.env.detected_fruits))

        f = open(logs_path + '/train_log.txt', 'a')
        f.write('Episode {} completed with perf {:.2f}!\n'.format(self.currEpisode, 100*self.env.detected_fruits))
        f.close()
    ...
    def plan_next_action(self, agent):
        ...
        node_util_inputs = agent.node_utils.reshape((n_nodes, 1))
        node_std_inputs = agent.node_std.reshape((n_nodes,1))
        node_neib_inputs = agent.neib_inputs.reshape((n_nodes,1))
        budget_inputs = self.calc_estimate_budget(agent)
        node_inputs = np.concatenate((agent.action_coords, node_util_inputs, node_std_inputs, node_neib_inputs), axis=1)
        node_inputs = torch.FloatTensor(node_inputs).unsqueeze(0).to(self.device) # (1, sample_size+2, 4)
        budget_inputs = torch.FloatTensor(budget_inputs).unsqueeze(0).to(self.device) # (1, sample_size+2, 1)
        # neib_encoding = torch.FloatTensor(neib_encoding).unsqueeze(0).to(self.device)
        ...
        agent.route.append(next_node_index.item())
        agent.mask = torch.zeros((1, self.sample_size, K_SIZE*len(FACING_ACTIONS)), dtype=torch.int64).to(self.device)
        # print('Agent {} planned'.format(agent.ID))
    ...
    def get_neib_inputs(self, agent_obj):
        features = list(agent_obj.curr_coord[0:3]) + agent_obj.neibs_coords
        return np.array(features, dtype=float)

    def calc_neib_inputs(self, agent_obj):
        all_dists = []
        for i, point_coord in enumerate(agent_obj.action_coords):
            min_dist = np.float('inf')
            for other_agent in self.env.agents:
                if other_agent.ID != agent_obj.ID:
                    dist_neib2point = min(np.linalg.norm(np.array([point_coord[0:3]]).reshape(1,3) - np.array(other_agent.node_coords).reshape(-1,3),axis=1))
                    if dist_neib2point < min_dist:
                        min_dist = dist_neib2point
            all_dists.append(min_dist)
        return np.asarray(all_dists).reshape(i+1, 1)
